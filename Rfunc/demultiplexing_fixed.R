...
clustering_based_demux = function(seurat_object, assay = "HTO", expected_doublet_rate = NULL,
  ...
  class= dplyr::if_else(seurat_object$sampleBC == "Negative" , "Negative",
                 dplyr::if_else(grepl("_",seurat_object$sampleBC) ,"Doublet" ,"Singlet"))
  class = factor(class)
                           ...
                           logfc.threshold = 0.1 ,knn = 20 )
{
  ...
  mtrx = as.matrix(mtrx) %>% t()

  #dist_mtrx = dist(mtrx %>% as.matrix() )
  nn = FindNeighbors(object = mtrx, k.param = knn, verbose = FALSE)
  snn = nn$snn
  ...
  # Set features to use only the features in the assay
  features_to_use <- rownames(seurat_object[[assay]])
  markers <- Seurat::FindAllMarkers(seurat_object,
                                    only.pos = TRUE,
                                    pseudocount.use = 0.1, # compatible with Seurat5
...
label_clusters <- function(seurat_object,markers)
{
  ...
  cell_classification.global <- cell_classification %>%
    dplyr::mutate(class= dplyr::if_else( sampleBC == "Negative" , "Negative",
                                 dplyr::if_else(grepl("_", sampleBC) ,"Doublet" ,"Singlet")))
