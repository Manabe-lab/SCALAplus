              ...
              tabPanel("Data manipulation", fluidRow(
                ...
                tabPanel("Demultiplexing", fluidRow(
                  ...
                  tags$div(style="background-color: #f0f8ff; padding: 10px; border-radius: 5px; margin-bottom: 10px;",
                    tags$h5(tags$b("推奨設定：")),
                    tags$p("✓ ", tags$b("deMULTIplex2"), " - 最高精度。物理的なタグ汚染を明示的にモデル化"),
                    tags$p("✓ ", tags$b("demuxmix"), " - 確率的評価が可能。エラー率を定量化"),
                    tags$p("✓ ", tags$b("MULTIseqDemux"), " - 標準手法として結果検証に使用"),
                    tags$p(style="color: #666; font-size: 0.9em;",
                           "複数の手法を併用することで、より信頼性の高い判定が可能です。",
                           "結果が一致しない細胞は慎重に扱うことを推奨します。")
                  ),
                  checkboxInput(inputId = "run_deMULTIplex2", label = "deMULTIplex2 (EM-GLM) - 第一選択", value = TRUE),
                  checkboxInput(inputId = "run_demuxmix", label = "demuxmix - 次点", value = TRUE),
                  checkboxInput(inputId = "run_MULTIseqDemux", label = "MULTIseqDemux (Seurat) - 併走対照", value = TRUE),
                  checkboxInput(inputId = "run_hashDemux", label = "hashDemux (clustering-based) - 追加検証", value = FALSE),
                  checkboxInput(inputId = "run_HTODemux", label = "HTODemux (Seurat) - 比較用", value = FALSE),
                  tags$hr(),
                  ...
                  tags$ul(
                    ...
                    tags$li("Method: EM algorithm with GLM-based modeling of tag contamination"),
                    tags$li("deMULTIplex2_type: singlet/multiplet/negative"),
                    tags$li("Generates tagHist and tagCallHeatmap diagnostic plots")
                  ),
                  tags$hr(),
