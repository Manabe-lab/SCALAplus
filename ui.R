...
ui <- dashboardPage(
  ...
  # Auto-reconnection and theme switch functionality JavaScript
  tags$head(
    tags$script(HTML('
  ...
  # Add disconnection overlay
  div(id = "disconnect-overlay",
      div(id = "disconnect-message", "Connection lost. Reconnecting...")
  ),

  # Toggle button displayed only when sidebar is closed
  tags$div(
                  ...
                  tabsetPanel(type = "tabs", id = "mainUploadTabs",
                       ...
                       tags$h3("Upload your h5ad file"),
                            ...
                            selected = "mouse"),
                                       ...
                                       # Options for h5ad file structure
                                       # ============================================================
                                       tags$h4(
                                       ...
                                       conditionalPanel(
                                         ...
                                         tags$div(
                                           ...
                                           tags$p(tags$strong("When COMPASS analysis is performed with metis:")),
                                           tags$ul(
                                             tags$li("COMPASS h5ad file can be loaded as a standard h5ad file"),
                                             tags$li("Reactions are treated as 'genes'"),
                                             tags$li("COMPASS scores are stored in the data slot"),
                                             tags$li(tags$em("'Load X as counts' is automatically turned OFF"), "(scores are already normalized)"),
                                             tags$li("Can be analyzed with FeaturePlot, VlnPlot, FindMarkers, etc.")
                                           ),
                             ...
                             tabPanel("10x spatial on v5",
                                       ...
                                       tags$h4("'outs' forlder to load multiple bins as layers or individual binned_outputs folder containing filtered_feature_bc_matrix.h5."),
                                        ...
                                        tags$h5("Select folder from the left column"),
                                        tags$br(),
                              ...
                              tabPanel("Batch",
                                ...
                                tags$h5('If batch effects are not observed in PCA or UMAP, batch correction is not necessary'),
                                tags$h5('fastMNN, Harmony, and Scenorama can be analyzed using RNA (normalized counts) or SCT data'),
                                tags$h6('For SCT, it may be appropriate to regress out percent.mt'),
                                tags$h5('In CLUSTERING, select the correct combination of Assay and Reduction, e.g., RNA-mnn, SCT-mnn'),
                                tags$h5('The Reduction used in Clustering and UMAP must also match'),
                                tags$h4('These methods are fundamentally for clustering and cannot be used for DEG analysis'),
                                tags$h4('For Vln Plot and DEG analysis, revert the active assay to RNA → UTILITY IDENTITY & ASSAY'),
tags$h5('https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1850-9, https://www.nature.com/articles/s41592-021-01336-8 ')
                ...
                box(
                  ...
                  conditionalPanel(
                    ...
                    tags$div(
                      style = "padding: 20px;",
                      tags$h5("Comparison of ambient RNA removal methods:"),
                      tags$div(
                        style = "background-color: #e7f3ff; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #2196F3;",
                        tags$p(tags$b(style = "color: #2196F3; font-size: 1.1em;", "CellBender - Standard first choice")),
                        tags$ul(
                          tags$li("Provides the most accurate estimate of background noise level and appropriately captures inter-cell variation"),
                          tags$li("Most robust performance against cell type composition and contamination sources"),
                          tags$li("Deep learning-based correction for all genes")
                        )
                      ),
                      tags$div(
                        style = "background-color: #f0f0f0; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #9E9E9E;",
                        tags$p(tags$b(style = "color: #666; font-size: 1.1em;", "SoupX - For rapid screening")),
                        tags$ul(
                          tags$li("Cannot capture inter-cell variation and tends to underestimate overall level"),
                          tags$li("Requires prior cell clustering"),
                          tags$li(tags$span(style = "color: #666;", "* Available in UTILITY DATA MANIPULATION menu"))
                        )
                      ),
                      tags$div(
                        style = "background-color: #f0f0f0; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #9E9E9E;",
                        tags$p(tags$b(style = "color: #666; font-size: 1.1em;", "scCDC - For highly contaminated marker genes")),
                        tags$ul(
                          tags$li("Detects and corrects only contamination-causing genes (GCGs), leaving other genes unchanged"),
                          tags$li("Effective for highly contaminated genes and avoids over-correction of housekeeping genes"),
                          tags$li("Requires prior cell clustering"),
                          tags$li("Empty droplet data not required"),
                          tags$li(tags$span(style = "color: #666;", "* Available in UTILITY DATA MANIPULATION menu"))
                        )
                      ),

                      tags$h5("About CellBender"),
                      tags$p("CellBender is a tool that removes technical artifacts (such as ambient RNA) from single-cell RNA-seq data."),

                      tags$h5("Processing steps:"),
                      tags$ol(
                        tags$li(tags$b("Verify 10x results:"), " Check output files from Cell Ranger (web_summary.html or metrics_summary.csv)."),
                        tags$li(tags$b("Determine Expected number of cells:"), " Check the value of 'Estimated Number of Cells' and enter that value. Accuracy is not critical; 2-fold error is acceptable."),
                        tags$li(tags$b("Determine Total droplets to include:"), " Generally set to 2-3 times Expected cells. Example: If Expected cells = 5,000, set Total droplets = 10,000-15,000."),
                        tags$li(tags$b("Select raw_feature_bc_matrix.h5:"), " Select the raw_feature_bc_matrix.h5 file (not filtered) from Cell Ranger output folder."),
                        tags$li(tags$b("Run:"), " Click the Run CellBender button to start processing. Processing takes 20-60 minutes.")
                      ),

                      tags$h5("Important parameters:"),
                      tags$div(
                        style = "background-color: #fff3cd; padding: 10px; border-radius: 5px; margin: 10px 0;",
                        tags$p(
                          style = "margin: 0;",
                          tags$b("Note:"), " Expected number of cells and Total droplets to include ",
                          tags$span(style = "color: #d9534f; font-weight: bold;", "must be adjusted for each dataset."),
                          " Do not use default values as-is."
                        )
                      ),

                      tags$h5("How to read parameters from 10x results:"),
                      tags$div(
                        style = "background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin: 10px 0;",
                        tags$p(tags$b("For Cell Ranger web_summary.html:")),
                        tags$ul(
                          tags$li(tags$b("Expected number of cells:"), " Use the value of 'Estimated Number of Cells' (2-fold error tolerable)"),
                          tags$li(tags$b("Total droplets to include:"), " Set to 2-3 times Expected cells"),
                          tags$li("Example: If Estimated Number of Cells = 5,247 → Expected cells = 5000-5500, Total droplets = 10000-15000")
                        ),
                        tags$p(tags$b("Other parameters (normally no change needed):")),
                        tags$ul(
                          tags$li(tags$b("FPR:"), " False Positive Rate = 0.01 (default)"),
                          tags$li(tags$b("Epochs:"), " Number of training iterations = 150 (default)"),
                          tags$li(tags$b("Learning rate:"), " Step size for optimization = 0.0001 (default)")
                        )
                      ),

                      tags$h5("How to read UMI curve:"),
                      tags$div(
                        ...
                        tags$p(
                          ...
                          "* UMI curve: X-axis is droplet ID (in order of count), Y-axis is UMI count. Sharp decline = actual cells, plateau = empty droplets."
                        )
                      ),
                      tags$ul(
                        tags$li(tags$b("Probable cells (sharp decline):"), " Droplets containing actual cells with high UMI count."),
                        tags$li(tags$b("Empty droplet plateau (plateau):"), " Empty droplets or those containing only ambient RNA."),
                        tags$li(tags$b("Setting Expected cells:"), " Number of droplets where sharp decline ends (example: ~5,000)."),
                        tags$li(tags$b("Setting Total droplets:"), " Range that includes empty droplet plateau sufficiently (example: 15,000-20,000).")
                      ),

                      tags$h5("Output files:"),
                      tags$p("CellBender generates filtered count matrix in .h5 format. After processing, load the cellbender_output_filtered.h5 file in the Upload tab."),
                      tags$p(tags$b("Note:"), " Processing takes 20-60 minutes depending on dataset size and number of epochs. While progress circle is displayed, you cannot move to other menus.")
                    )
                  ),
      ...
      tabItem(tabName = "file",
       ...
       shinyDirButton('localFolderDelete', 'Select a folder', 'Please select a folder', viewtype='list'),
        ...
        actionButton(inputId = "confirmRefresh", label = "Refresh the session", class="btn btn-danger"),
            ...
            box(width = 6,
            tags$h4("qs2 format is standard.\n"),
            tags$br(),
            tags$h4("When converting to Anndata (h5ad) file, the data from the active assay is output.\n"),
            tags$h4("Usually select RNA.\n"),
                                             ),
              ...
              fluidRow(
                box(
                 ...
                 tags$br(),
                  ...
                  tags$h4("If minimum cell count is set to 0, it will match the cell count of the smallest cluster in active identity"),
        ...
        tabsetPanel(type = "tabs",
              tabPanel("Data manipulation", fluidRow(
                ...
                tabPanel("Demultiplexing", fluidRow(
                  ...
                  tags$h4("Algorithm Selection"),
                  tags$div(style="background-color: #f0f8ff; padding: 10px; border-radius: 5px; margin-bottom: 10px;",
                    tags$h5(tags$b("Operational Strategy:"), "Primary + Sensitivity + Baseline roles"),
                    tags$p(tags$b("Primary (main analysis candidates)"), " - Select based on data conditions"),
                    tags$p("- ", tags$b("demuxmix"), " - Mixture model. Can quantitatively evaluate error probability of assignment (Galembert et al., Bioinformatics 2023)"),
                    tags$p("- ", tags$b("hashDemux"), " - Clustering-based. Robustness reported by independent benchmarks (BFG 2025)"),
                    tags$p(tags$b("Sensitivity (recovery-focused, additional validation)")),
                    tags$p("- ", tags$b("deMULTIplex2"), " - Modeling tag cross-contamination with EM-GLM. ",
                           "Can rescue cells that become Negative with other methods, but additional recovered cells require diagnostic plot validation (Zeng et al., Genome Biology 2024)"),
                    tags$p(tags$b("Baseline (for comparison)")),
                    tags$p("- ", tags$b("MULTIseqDemux / HTODemux"), " - Seurat standard methods. Used as comparison baseline"),
                    tags$p(style="color: #666; font-size: 0.9em;",
                           "Purpose of using multiple methods: Flag discordant cells (cells inconsistent between methods) and ",
                           "subject them to holdout, exclusion, or sensitivity analysis. Accuracy does not automatically increase.")
                  ),
                  checkboxInput(inputId = "run_demuxmix", label = "demuxmix (mixture model) - primary candidate", value = TRUE),
                  checkboxInput(inputId = "run_hashDemux", label = "hashDemux (clustering-based) - primary candidate", value = TRUE),
                  checkboxInput(inputId = "run_deMULTIplex2", label = "deMULTIplex2 (EM-GLM) - sensitivity / recovery-focused", value = FALSE),
                  ...
                  tags$ul(
                    ...
                    tags$li("Quality evaluation of additionally recovered cells: posterior concentrated around 0.5 → uncertain classification, scattered on UMAP → suspicion of background noise")
                  ),
                  tags$hr(),
                ...
                tabPanel("scCDC", fluidRow(
                  ...
                  tags$div(
                    ...
                    tags$p(style = "margin: 0; font-weight: bold;", "For data merged from separate batches/libraries:"),
                    tags$p(style = "margin: 5px 0;",
                      "Since each sample has its own ambient RNA pool, ",
                      tags$span(style = "color: #d9534f; font-weight: bold;", "run scCDC individually on each sample before merging.")
                    )
                  ),
                  ...
                  tags$div(
                    ...
                    tags$p(style = "margin: 0; font-weight: bold;", "For multiplexed data (single library):"),
                    tags$p(style = "margin: 5px 0;",
                      "Samples multiplexed with Hashtag/MULTI-seq etc. ",
                      tags$b("share the same ambient RNA pool"),
                      ", so scCDC can be run directly on demultiplexed data."
                    ),
                    tags$p(style = "margin: 5px 0; color: #155724;",
                      "→ Use ", tags$b("orig.ident"), " as identity to detect contamination between samples"
                    )
                  ),
                  tags$hr(),
                  ...
                  selectInput(inputId = "scCDCIdent",
                             ...
                             c("Cluster" = "seurat_clusters")),
                  tags$p(style = "color: #666; font-size: 0.9em;",
                         "For multiplexed data: orig.ident (by sample) recommended. Detects contamination between samples in the same library."),
                  tags$p(style = "color: #666; font-size: 0.9em;",
                         "To detect contamination between cell types: use cell type clusters."),
                  tags$hr(),
                  ...
                  tags$div(
                    ...
                    fluidRow(
                      column(4,
                        ...
                        tags$p(style = "color: #666; font-size: 0.8em;",
                               "Threshold for identifying contamination genes. Detects genes recognized as contamination in clusters above this proportion.",
                               tags$b("Lower = detect more↑"), "(default: 0.5 = 50%)")
                      ),
                      column(4,
                        ...
                        tags$p(style = "color: #666; font-size: 0.8em;",
                               "Clusters with fewer than this number of cells are excluded from analysis.",
                               tags$b("Lower = include small clusters"), "(default: 100)")
                      ),
                      column(4,
                        ...
                        tags$p(style = "color: #666; font-size: 0.8em;",
                               "Expression rate threshold in each cluster. Candidate genes below this are excluded.",
                               tags$b("Lower = detect lower expression"), "(default: 0.2 = 20%)")
                      )
                    ),
                    ...
                    fluidRow(
                      column(6,
                        ...
                        tags$p(style = "color: #666; font-size: 0.8em;",
                               "AUROC threshold for determining boundary between GCG positive/negative clusters.",
                               tags$b("Lower = more correction targets"), "(default: 0.9 = 90%)")
                      ),
                      column(6,
                        ...
                        tags$p(style = "color: #666; font-size: 0.8em;",
                               "Clusters with fewer than this number of cells excluded during correction. (default: 50)")
                      )
                    ),
                    tags$hr(),
                    tags$p(style = "color: #856404; font-size: 0.9em;",
                           tags$b("Tip:"), " If detection count is low, try lowering restriction_factor to 0.3 and min.cell to 50.")
                  ),
                  tags$hr(),
                  ...
                  tags$div(
                    ...
                    tags$p(style = "margin: 0; font-weight: bold; color: #721c24;", "Exclude biologically important genes:"),
                    tags$p(style = "margin: 5px 0; color: #721c24; font-size: 0.9em;",
                      "scCDC may mistakenly recognize cell type-specific genes as 'contamination'.",
                      "Examples: Nr2f2 (venous marker), Gja4 (arterial marker), etc."
                    )
                  ),
                  ...
                  tags$p(style = "color: #666; font-size: 0.85em;",
                         "These genes are excluded from the GCG list and will not be corrected."),
                  tags$hr(),
                ...
                tags$details(
                  tags$summary(tags$b("Decision guide for ALRA batch splitting (click to expand)")),
                  tags$ul(
                    tags$li(tags$b("General rule not established"), ": No systematic comparative studies between post-merge ALRA vs batch-specific ALRA"),
                    tags$li(tags$b("Original paper uses post-merge ALRA"), ": All examples in Linderman et al. 2022 follow 'merge→normalize→ALRA' procedure"),
                    tags$li(tags$b("Split effective for heterogeneous populations"), ": Valyaeva et al. 2026 presents cell type-specific ALRA strategy (for non-zero fraction goal)"),
                    tags$li(tags$b("Report of condition difference masking"), ": Observation in GitHub Issue #5 that 'treatment/control difference is lost' (but single user, unverified)"),
                    tags$li(tags$b("Practical conclusion"), ": If condition×batch are not confounded, start with post-merge ALRA; if confounded or condition difference is critical, try both and compare (judge by logFC, pseudobulk consistency, etc.)")
                  )
                ),
                tags$hr(),
                tags$h3("MAGIC"),
               ...
               tags$h3("deepImpute"),
                ...
                tabPanel("Data to reducition", fluidRow(
                  ...
                  tags$h4("Use data itself (such as gene sets) for UMAP calculation"),
                  tags$h5("UMAP function calculates based on dimensionality-reduced data like PCA. To calculate UMAP directly from data, copy data to a dimensionality reduction slot called GSET."),
                  tags$h5("When using all GSET data for UMAP and other analyses, check 'Use all dimensions to calculate?'."),
                  ...
                  checkboxInput(inputId = "ignoreOrderMeta", label = "Add metadata in order regardless of cell names", value = FALSE),
                  ...
                  tags$h5("Cell names must match. It is relatively easy to correct cell names in downloaded metadata. Existing metadata will not be overwritten."),
                  tags$h5("In-order option: Ignores cell names and adds metadata in row number order. Returns error if cell count differs."),

                  tags$hr(),
                  tags$h3("Load cellranger annotate results"),
                  tags$h5("Select cell_types.csv from cellranger annotate on the server and add to metadata."),
                  ...
                  tags$h5("Download file names and correct them in the same format."),
               ...
               tabPanel("Augur", fluidRow(
                 box(status = "info", solidHeader = TRUE, title = "Cell Type Prioritization (Augur)", width = 12,
                     tags$h4("Augur: Evaluate cell type responsiveness to experimental conditions"),
                     tags$p("Uses machine learning (random forest classifier) to evaluate how well each cell type can distinguish experimental conditions (stimulation vs control) using AUC values."),
                     tags$p("Cell types with higher AUC indicate stronger response to experimental conditions."),
                     tags$hr(),
                     ...
                     fluidRow(
                       column(4,
                         ...
                         tags$p(style = "color: #666; font-size: 0.8em;",
                                "Number of subsampling iterations. More is more stable but takes longer (default: 50)")
                       ),
                       column(4,
                         numericInput("augurMinCells", "min_cells:", value = 20, min = 5, max = 100, step = 5),
                         tags$p(style = "color: #666; font-size: 0.8em;",
                                "Minimum cells per cell type. Cell types with fewer cells are skipped (default: 20)")
                       ),
                       column(4,
                         numericInput("augurVarQuantile", "var_quantile:", value = 0.5, min = 0.1, max = 1.0, step = 0.1),
                         tags$p(style = "color: #666; font-size: 0.8em;",
                                "Proportion of highly variable genes to use (default: 0.5 = top 50%)")
                       )
                     ),
                                              ...
                                              column(width = 4,
                                                box(width = 12, status = "info", solidHeader = TRUE,
                                                    ...
                                                    checkboxInput("pcCovIncludeInteractions",
                                                                 ...
                                                                 value = FALSE),
                                                    tags$small("When checked, only the interaction term between Covariate 1 and Covariate 2 is added to the model. Other covariates include only main effects."),
                                                    tags$hr(),
                                                    ...
                                                    tags$div(style="background-color: #fffacd; padding: 12px; margin-top: 15px; border-radius: 5px; border-left: 4px solid #ffa500;",
                                                      tags$h4("Typical usage example (scRNA-seq analysis)", style="margin-top: 0;"),
                                                      tags$p(tags$b("Assumptions:")),
                                                      tags$ul(style="margin-bottom: 8px;",
                                                        tags$li(tags$b("sample"), ": Mouse individual/donor ID (cells from same sample are non-independent)"),
                                                        tags$li(tags$b("sex"), ": Biological factor (male/female, etc.)"),
                                                        tags$li(tags$b("cell.ident"), ": Cell subtype/cluster")
                                                      ),
                                                      tags$p(tags$b("How to set up the model:")),
                                                      tags$ol(style="margin-bottom: 8px;",
                                                        tags$li(tags$b("Subtype effect only"), ": Covariate 1 = cell.ident → Identify axes explained by subtype such as PC1/PC2"),
                                                        tags$li(tags$b("Main effect model"), ": Covariate 1 = sex, Covariate 2 = cell.ident → Evaluate average sex difference adjusted for subtype"),
                                                        tags$li(tags$b("Interaction model"), ": Plus check 'Covariate 1 × 2' above → Test whether direction or magnitude of sex difference differs by subtype (e.g., arterial ♀>♂, venous ♂>♀)")
                                                      ),
                                                      tags$p(tags$b("Essential:"), style="color: #d9534f; margin-bottom: 5px;"),
                                                      tags$ul(style="margin-bottom: 5px;",
                                                        tags$li(tags$b("Always specify sample in Random effect 1"), "(adjust for correlation between cells from same sample)"),
                                                        tags$li("Presence of interaction is determined by ", tags$b("lrt_pvalue"), " in ", tags$b("Model Comparison tab"), "(significant if p < 0.05)")
                                                      ),
                                                      tags$p(tags$small("* Simple linear models (lm) without specifying Random effect ignore cell non-independence, resulting in overly significant p-values. To obtain biologically sound conclusions, always use mixed-effect models (lmer) with random effects."),
                                                    ...
                                                    tabsetPanel(type = "tabs",
                                                      tabPanel("Summary Statistics",
                                                               ...
                                                               tags$div(style="background-color: #f0f8ff; padding: 10px; margin-bottom: 10px; border-radius: 5px;",
                                                                 ...
                                                                 tags$ul(
                                                                   tags$li(tags$b("fixed_coef_[covariate]"), ": Regression coefficient (slope for continuous variables, effect of one level for categorical variables)"),
                                                                   tags$li(tags$b("fixed_pval_[covariate]"), ": P-value of individual coefficient (", tags$i("for categorical variables, not overall significance"), ")"),
                                                                   tags$li(tags$b("fixed_global_pval_[covariate]"), ": ", tags$span(style="color: #d9534f; font-weight: bold;", "P-value for entire factor (calculated from F-test, recommended)")),
                                                                   tags$li(tags$b("partial_r2_[covariate]"), ": Partial R² (proportion of PC variance explained by this covariate)"),
                                                                   tags$li(tags$b("fixed_r2"), ": R² of model including all covariates (overall model explanatory power)")
                                                                 ),
                                                                 tags$p(tags$b("Statistically important notes:"), style="margin-top: 10px; color: #d9534f;"),
                                                                 tags$ul(
                                                                   tags$li(tags$b("Categorical variables (batch, cell_type, etc.)"), ": fixed_pval is p-value of only one level. Use ", tags$b("fixed_global_pval")),
                                                                   tags$li(tags$b("Partial R² (with interaction)"), ": Partial R² of main effect includes ", tags$i("all interactions involving that factor"), "(e.g., sexPartial R² = sex main effect + sex:cell.ident interaction)"),
                                                                   tags$li(tags$b("Heatmap and plot"), ": For categorical variables, display global p-value (significance of entire factor)")
                                                                 ),
                                                                 tags$p(tags$b("Interpretation points:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li("High Partial R² (0.1 or more) + small global p-value (< 0.05) = strong association between covariate and PC"),
                                                                   tags$li("Example: PC_1 partial_r2_cell_type = 0.35 → 35% of PC1 variance is explained by cell type"),
                                                                   tags$li("PCs with high Partial R² are strongly affected by that biological/technical factor")
                                                                 )
                                                               ),
                                                      ...
                                                      tabPanel("Model Comparison",
                                                               tags$h4("Comparison of full model vs reduced model"),
                                                               tags$p("Compare model fit when each covariate is excluded"),
                                                               tags$br(),
                                                               tags$div(style="background-color: #fff3cd; padding: 10px; margin-bottom: 10px; border-radius: 5px;",
                                                                 tags$h5("How to interpret results (linear model):"),
                                                                 tags$ul(
                                                                   tags$li(tags$b("term_removed"), ": Term excluded (main effect or interaction term)"),
                                                                   tags$li(tags$b("term_type"), ": Type of term (main_effect: main effect, interaction: interaction)"),
                                                                   tags$li(tags$b("full_model_r2"), ": R² of model with all terms"),
                                                                   tags$li(tags$b("reduced_model_r2"), ": R² of model without that term"),
                                                                   tags$li(tags$b("partial_r2"), ": Proportion of variance explained by that term (full - reduced)"),
                                                                   tags$li(tags$b("delta_aic"), ": AIC change for linear model (reduced - full)"),
                                                                   tags$li(tags$b("f_statistic"), ": F-statistic (ANOVA F-test)"),
                                                                   tags$li(tags$b("f_test_pvalue"), ": P-value of F-test (does adding term significantly improve model?)")
                                                                 ),
                                                                 tags$h5("How to interpret results (mixed-effect model, only with random effect specified):", style="margin-top: 15px;"),
                                                                 tags$ul(
                                                                   tags$li(tags$b("mixed_r2m"), ": Marginal R² (variance explained by fixed effects only)"),
                                                                   tags$li(tags$b("mixed_r2c"), ": Conditional R² (variance explained by fixed + random effects)"),
                                                                   tags$li(tags$b("mixed_delta_aic"), ": AIC change for mixed-effect model"),
                                                                   tags$li(tags$b("chisq_statistic"), ": χ² statistic (likelihood ratio test/LRT)"),
                                                                   tags$li(tags$b("lrt_pvalue"), ": P-value of LRT (is term addition significant in mixed-effect model?)")
                                                                 ),
                                                                 tags$p(tags$b("Interpretation points:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li(tags$b("Main effect (main_effect)"), "→ Direct effect of covariate"),
                                                                   tags$li(tags$b("Interaction (interaction)"), "→ Effect from combination of two covariates"),
                                                                   tags$li(tags$b("Linear model:"), "Treat all cells as independent"),
                                                                   tags$ul(
                                                                     tags$li(tags$b("High Partial R²"), "→ Term strongly explains PC (important)"),
                                                                     tags$li(tags$b("F-test p-value small (< 0.05)"), "→ Adding term significantly improves model (important)")
                                                                   ),
                                                                   tags$li(tags$b("Mixed-effect model:"), "Consider grouping structure like donors"),
                                                                   tags$ul(
                                                                     tags$li(tags$b("LRT p-value small (< 0.05)"), "→ Term significantly contributes even considering random effects (important)"),
                                                                     tags$li(tags$b("mixed_r2m (marginal R²)"), "→ Explanatory power of fixed effects only"),
                                                                     tags$li(tags$b("mixed_r2c (conditional R²)"), "→ Explanatory power of fixed + random effects"),
                                                                     tags$li("mixed_r2c - mixed_r2m → Contribution of random effects (donors, etc.)")
                                                                   ),
                                                                   tags$li("Example: cell_type:batch interaction with partial_r2 = 0.15, lrt_pvalue < 0.001 → Batch effect differs by cell type, significant even considering inter-donor variation")
                                                                 ),
                                                                 tags$p(tags$b("Concrete usage:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li("Verify batch effects: PCs with high Partial R² may need batch correction"),
                                                                   tags$li("Identify biological factors: PCs with high cell type Partial R² capture cell differences"),
                                                                   tags$li("Evaluate interactions: High cell_type:treatment Partial R² → treatment effect is cell type-dependent"),
                                                                   tags$li("Impact on clustering: Consider excluding PCs explained by unnecessary terms (like batch)")
                                                                 )
                                                               ),
                                                      ...
                                                      tabPanel("Partial R² Heatmap",
                                                               tags$div(style="background-color: #e7f3ff; padding: 10px; margin-bottom: 10px; border-radius: 5px;",
                                                                 tags$h5("How to read heatmap:"),
                                                                 tags$ul(
                                                                   tags$li("Darker color (red) means covariate more strongly explains PC"),
                                                                   tags$li("Numbers are Partial R² (range 0-1, closer to 1 means stronger association)"),
                                                                   tags$li("Vertical axis: principal components (PC), horizontal axis: covariates (cell type, batch, etc.)")
                                                                 ),
                                                                 tags$p(tags$b("How to use:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li("Look for red areas → that PC×covariate combination is important"),
                                                                   tags$li("Example: PC1 is red for cell_type → PC1 mainly captures cell type differences"),
                                                                   tags$li("Example: PC3 is red for batch → PC3 is affected by batch effect (note carefully)")
                                                                 )
                                                               ),
                                                      ...
                                                      tabPanel("P-value Heatmap",
                                                               tags$div(style="background-color: #ffe7e7; padding: 10px; margin-bottom: 10px; border-radius: 5px;",
                                                                 tags$h5("How to read P-value heatmap:"),
                                                                 tags$ul(
                                                                   tags$li("Darker color (dark red) indicates statistically significant association"),
                                                                   tags$li("Numbers show -log10(p-value) (larger = more significant)"),
                                                                   tags$li(tags$b("* mark"), "present = p < 0.05 (statistically significant)")
                                                                 ),
                                                                 tags$p(tags$b("Interpretation guide:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li("-log10(p) > 1.3 (p < 0.05) → Significant association"),
                                                                   tags$li("-log10(p) > 2 (p < 0.01) → Strong significance"),
                                                                   tags$li("-log10(p) > 3 (p < 0.001) → Very strong significance"),
                                                                   tags$li("* mark present AND red in Partial R² heatmap → Definitely important association")
                                                                 ),
                                                                 tags$p(tags$b("Notes:"), style="margin-top: 10px;"),
                                                                 tags$ul(
                                                                   tags$li("Check not just p-value but also Partial R² (effect size)"),
                                                                   tags$li("With large sample size, even small effects can become significant")
                                                                 )
                                                               ),
                                   ...
                                   downloadButton(outputId = "barplotdownloaderPNG",label = "download as png"),
                                     ...
                                     downloadButton(outputId = "clusterTableExport", label = "download table"),
                                                 ...
                                                 div(id="clusterBarplot_loader",
                                                      shinycssloaders::withSpinner(
                                                        uiOutput("clusterBarplotUI")  # use uiOutput instead of plotOutput
                                                      )
                                                    ),
                                                 tags$br(),
                                                 tags$br(),
                       ...
                       conditionalPanel(
                         ...
                         sliderInput(inputId = "umapOutComponents", label = "Number of dimensions to calculate:", min = 2, max = 50, value = 2, step = 1)%>%
                           shinyInput_label_embed(
                             shiny_iconlink() %>%
                               bs_embed_popover(
                                 title = "If PHATE is selected, the runtime increases when a value > 3 is used.\nFor pca, umap, dfm, increasing dimensions does not change lower-dimensional values.", placement = "bottom")),
                   ...
                   conditionalPanel(
                    ...
                    conditionalPanel(
                        ...
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "n_neighbors is the number of nearby points considered when embedding each data point. Larger values emphasize overall structure, smaller values preserve local structure. Typically 2-100. Seurat default: 30", placement = "left"
                            )
                        ),
                        ...
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Indicates minimum distance between points after dimensionality reduction. Small = dense points, large = spread points preserving topological structure. Seurat default: 0.3", placement = "left"
                            )
                        ),
                        ...
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Expresses 'spread' of data points to more accurately reflect density structure of original data.",
                            placement = "left"
                            )
                    )),
                    ...
                    conditionalPanel(
                            ...
                            shinyInput_label_embed(
                              shiny_iconlink() %>%
                                bs_embed_popover(
                                  title = "Higher values prioritize density preservation over UMAP objective. Adjusts importance of density information. Larger values better reflect original data density structure.",
                                  placement = "left"
                                )
                            ),
                            ...
                            shinyInput_label_embed(
                              shiny_iconlink() %>%
                                bs_embed_popover(
                                  title = "Higher values place more emphasis on preserving density information from early in optimization. Of all iterations, the specified proportion of iterations includes density preservation term in objective function, remaining use only normal UMAP objective. Large value: prioritize density preservation, strongly reflect original data density structure. Small value: result closer to normal UMAP, prioritize topology structure preservation.", placement = "left"
                                )
                    )),
                    ...
                    conditionalPanel(
                          ...
                          tags$h5("Elastic Embedding distribution is affected by the number of dimensions used."),
                          tags$h5("Start with 25-35 PCs, increase when identifying rare cell populations, decrease when there is much noise."),
                    ...
                    conditionalPanel(
                      ...
                      numericInput("pacmap_n_neighbors", "n_neighbors", min = 2, max = 200, value = 10, step = 1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of nearby points. Like UMAP n_neighbors, affects preservation of local structure. Default: 10", placement = "left"
                            )
                        ),
                      numericInput("pacmap_MN_ratio", "MN_ratio", min = 0.1, max = 2, value = 0.5, step = 0.1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Mid-near ratio. Weight of mid-distance pairs. Large values emphasize global structure. Default: 0.5", placement = "left"
                            )
                        ),
                      numericInput("pacmap_FP_ratio", "FP_ratio", min = 0.5, max = 5, value = 2, step = 0.1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Further pairs ratio. Weight of far-distance pairs. Large values emphasize cluster separation. Default: 2", placement = "left"
                            )
                        )
                    ),
                    ...
                    conditionalPanel(
                      ...
                      tags$h5("Calculates layout using SNN/KNN graph."),
                      numericInput("fa2_iterations", "iterations", min = 100, max = 5000, value = 1000, step = 100) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of algorithm iterations. More iterations = more stable but increased computation time. Default: 1000", placement = "left"
                            )
                        ),
                      checkboxInput("fa2_linlog", label = "LinLog mode (emphasized cluster separation)", value = FALSE),
                      checkboxInput("fa2_prevent_overlap", label = "Prevent overlap", value = FALSE)
                    ),
                    ...
                    conditionalPanel(
                      ...
                      numericInput("trimap_n_inliers", "n_inliers", min = 2, max = 50, value = 10, step = 1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of nearby points for each point. Affects preservation of local structure. Default: 10", placement = "left"
                            )
                        ),
                      numericInput("trimap_n_outliers", "n_outliers", min = 2, max = 20, value = 5, step = 1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of outlier points for each point. Affects preservation of global structure. Default: 5", placement = "left"
                            )
                        ),
                      numericInput("trimap_n_random", "n_random", min = 1, max = 10, value = 5, step = 1) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of random triplets. Default: 5", placement = "left"
                            )
                        ),
                      numericInput("trimap_n_iters", "n_iters", min = 100, max = 1000, value = 400, step = 50) %>%
                        shinyInput_label_embed(
                          shiny_iconlink() %>%
                            bs_embed_popover(
                              title = "Number of optimization iterations. Default: 400", placement = "left"
                            )
                        )
                    ),
...
tags$br(),
                      ...
                      tags$h4("Additional options:"),
                         ...
                         conditionalPanel(
                           ...
                           tags$div(
                             ...
                             checkboxInput("umapHighlightFront", label = "Draw highlighted clusters in front", value = FALSE)
                           )
                         ),
                         ...
                         selectInput("umaplegendtextColor", "Legend font color:", c('black','white','gray','lightgray','azure','cornsilk','cyan','red','orange'),
                            selected = 'black'),
                        # checkboxInput("umaplegendBack", label = "White background for labels?", value = FALSE), not yet implemented correctly
                         tags$hr(),
                       ...
                       checkboxInput("umapReverseY", label = "Reverse Y axis?", value = FALSE),
                  ...
                  selectInput("feature3DColor", "FeaturePlot color scheme:",
                       ...
                       "Blues" = "Blues",               # changed default to Blues
                       "RdBu" = "RdBu",                 # changed Zissou1 to RdBu
                       "YlOrRd" = "YlOrRd",             # alternative to Reds
                       "YlGnBu" = "YlGnBu",             # alternative to Greens
                       "Viridis" = "Viridis",
                       "Cividis" = "Cividis",
                       "Inferno" = "Inferno",
                       "Plasma" = "Plasma"),
...
#                            selected = 'black'),
                    ...
                    box(width = 9, status = "info", solidHeader = TRUE, title = "Plot", height = "1200px",
                          ...
                          )), #end of scRNA-seq tab

                          tabPanel("Cell Selection",
                                   fluidRow(
                                     box(width = 3, status = "info", solidHeader = TRUE,
                                         title = "Lasso Cell Selection",
                                         selectInput("lassoReduction", "Reduction:", c("reduction" = "umap")),
                                         selectInput("lassoColorBy", "Color by:", c("Cluster" = "seurat_clusters")),
                                         selectInput("lassoColorPalette", "Color palette:",
                                                     c("Set1", "Set2", "Set3", "Paired", "Dark2", "Accent", "Spectral",
                                                       'stallion','stallion2','calm','kelly','alphabet','bear','ironMan','circus','paired',
                                                       'grove','summerNight','zissou','Zissou1Continuous', 'darjeeling','rushmore','captain'),
                                                     selected = 'Set1'),
                                         numericInput("lassoDotSize", "Dot size:", min = 0.5, max = 20, value = 3, step = 0.5),
                                         actionButton("lassoUpdatePlot", "Update plot", class = "btn btn-warning"),
                                         tags$hr(),
                                         tags$h4("Selection Actions"),
                                         textInput("lassoNewIdent", "New ident name:", placeholder = "e.g. CellType_A"),
                                         actionButton("lassoAssignIdent", "Assign ident to selected cells", class = "btn btn-primary"),
                                         tags$br(), tags$br(),
                                         actionButton("lassoSubsetKeep", "Keep selected cells only", class = "btn btn-success"),
                                         tags$br(), tags$br(),
                                         actionButton("lassoSubsetRemove", "Remove selected cells", class = "btn btn-danger"),
                                         tags$hr(),
                                         verbatimTextOutput("lassoSelectionInfo")
                                     ),
                                     box(width = 9, status = "info", solidHeader = TRUE,
                                         title = "Interactive Plot (use lasso tool to select cells)",
                                         shinycssloaders::withSpinner(
                                           plotlyOutput("lassoPlot", height = "700px")
                                         )
                                     )
                                   )
                          ), #end of Cell Selection tab
                                     ...
                                     box(width = 3, status = "info", solidHeader = TRUE,
                                         ...
                                         sliderInput(inputId = "umapOutComponentsATAC", label = "Number of dimensions to fit output:", min = 2, max = 100, value = 3, step = 1)%>%
                                           shinyInput_label_embed(
                                             shiny_iconlink() %>%
                                               bs_embed_popover(
                                                 title = "Please note that tSNE doesn't return more than 2 dimensions. For UMAP and Dfm, results don't change even if dimensions are changed.", placement = "bottom"
                                               )
                                           ),
               ...
               #  numericInput("MultifindMarkersHeight", "Plot height:", min = 200, max = 2400, value = 550, step = 10),
                ...
                tags$div(style = "display: flex; align-items: center;",
                  ...
                  tags$span(
                    ...
                    title = "Add random noise to expression values. Effective when cells with same expression values overlap and are hard to see. Noise is a small value around 1% of max expression. Not applied when displaying data points in stacked violin plots.",
                    `data-toggle` = "tooltip",
...
box(width = 4, status = "info", solidHeader = TRUE,
    ...
    # Add tabsetPanel here to create two tabs
    tabsetPanel(
      # Single Sample tab
      ...
      # Multiple Samples tab
      tabPanel("Multiple Samples",
               # NEBULA/glmmTMB analysis section
              ...
              numericInput(inputId = "NEBULAMinPctDEG", label = "Minimum fraction of cells expressing genes (min.pct)", min = 0, max = 1, value = 0.1, step = 0.01)%>%
               ...
               tags$h5("Select variables such as cell detection rate (CDR) and batch effects"),
               ...
               # MAST random effect analysis section
               ...
               tags$h5("First create Cluster x condition identity → UTILITY IDENTITY & ASSAY → AxB"),
               tags$h5("Select cells included in each group. Example: 1_WT1, 1_WT2 vs. 1_KO1, 1_KO2"),
               ...
               tags$h5("CDR is recommended for MAST"),
                                     ...
                                     box(
                                       ...
                                       column(
                                         ...
                                         tags$div(style = "margin-top: 10px;",
                                           ...
                                           tags$hr(),
                                           tags$h5(tags$b("Recommended workflow (DoubletFinder):"), style = "color: #d9534f;"),
                                           tags$p("1. ", tags$b("First perform basic QC:"), "Remove extremely low-quality cells (ultra-low UMI, ultra-low gene count, etc.) and debris"),
                                           tags$p("2. ", tags$b("Perform normalization and PCA:"), "Normalization → PCA → DoubletFinder on filtered object"),
                                           tags$p("3. ", tags$b("Important note:"), style = "color: #d9534f;", "Avoid overly strict QC. This can remove actual doublet structures and reduce detection accuracy"),
                                           tags$p("4. ", tags$b("After DoubletFinder execution:"), "Determine final QC threshold based on doublet scores and perform additional filtering (mitochondria-high-expression cells, etc.)"),
                                           tags$p("5. ", tags$b("For multiple samples:"), "Doublets occur only within same physical capture. If captured on separate flow cells/runs, split samples and run individually. Multiple samples mixed with hashtags (same capture) can be run together")
                                         )
                                       )
                                       )
                                   ),
                                     ...
                                     box(
                                       ...
                                       column(
                                         ...
                                         tags$div(style = "margin-top: 10px;",
                                           ...
                                           tags$hr(),
                                           tags$h5(tags$b("Recommended workflow (scDblFinder):"), style = "color: #5bc0de;"),
                                           tags$p("1. ", tags$b("Perform minimal QC only:"), "Remove only empty droplets and cells extremely close to zero (obvious debris)"),
                                           tags$p("2. ", tags$b("Run on 'almost raw' data:"), "Run scDblFinder on ", tags$b("lightly filtered"), " object with normalization and PCA already performed"),
                                           tags$p("3. ", tags$b("After scDblFinder execution:"), "Once doublet scores are obtained, perform standard QC filtering (mitochondria-high-expression cells, low-quality cells, etc.)"),
                                           tags$p("4. ", tags$b("Difference from DoubletFinder:"), style = "color: #5bc0de;", "scDblFinder runs at early QC stage. DoubletFinder runs on high-quality data after QC"),
                                           tags$p("5. ", tags$b("About sample splitting:"), "Doublets occur only within same physical capture (same flow cell/10x run). Multiple samples mixed with hashtags are treated as one capture (no splitting needed). Only split with batch/capture column if captured on separate flow cells/runs")
                                         )
                                       )
                                     )
                                   )
                          ...
                          tabPanel("HemaScribe",
                                   fluidRow(
                                     ...
                                     box(
                                       ...
                                       tags$h5("Cell classification is added to metadata (ident)"),
                                       tags$h5("Elastic Embedding, which clearly shows differentiation trajectories, is registered as EE"),
                                       tags$h5("Select EE in Plot reduction"),
                                       tags$h5("Pseudotime and hematopoietic score can be selected as Gene signature in Feature Plot"),
                                       tags$hr(),
                                       tags$h5("HemaScape Results Explanation:"),
                                       tags$h4("1. density_cluster_pred (density clusters)"),
                                       tags$p("Represents ", tags$strong("stable cell states"), " in transcriptional state space"),
                                       tags$ul(
                                         tags$li(tags$strong("High-density regions"), " identified by DensityPath algorithm"),
                                         tags$li("Express ", tags$strong("important intermediate points"), " and ", tags$strong("decision points"), " in hematopoietic differentiation"),
                                         tags$li("Stable states where cells reside for relatively long periods"),
                                         tags$li(tags$strong("Examples"), ": HSC state, MPP state, lineage-committed progenitor state, etc.")
                                       ),
                                       tags$h4("2. branch_pred (differentiation lineage)"),
                                       tags$p("Represents ", tags$strong("major lineages"), " of hematopoietic differentiation"),
                                       tags$ul(
                                         tags$li("Reflects ", tags$strong("major fate decisions"), " in hematopoietic differentiation"),
                                         tags$li(tags$strong("Major differentiation pathways"), ": ",
                                                tags$ul(
                                                  tags$li("Myeloid (myeloid lineage)"),
                                                  tags$li("Lymphoid (lymphoid lineage)"),
                                                  tags$li("Megakaryocyte/Erythroid (megakaryocyte/erythroid lineage)")
                                                )),
                                         tags$li("Integrated with HemaScribe cell type annotation")
                                       ),
                                       tags$h4("3. branch_segment_clusters_pred (differentiation segments)"),
                                       tags$p(tags$strong("Finer differentiation stages"), " within each lineage"),
                                       tags$ul(
                                         tags$li("Further subdivides branch_pred"),
                                         tags$li("Captures ", tags$strong("differentiation progression within the same lineage")),
                                         tags$li(tags$strong("Example"), ": Step-wise differentiation from GMP→GP→mature granulocytes in Myeloid lineage")
                                       ),
                                       tags$h4("4. pseudotime_pred (pseudotime)"),
                                       tags$p(tags$strong("Differentiation progression"), " from HSC"),
                                       tags$ul(
                                         tags$li("Differentiation progression with HSC as starting point (pseudotime = 0)"),
                                         tags$li("Larger values = more advanced differentiation state"),
                                         tags$li("Shows position on the timeline of hematopoietic differentiation")
                                       ),
                                       tags$h5("Biological interpretation"),
                                       tags$ul(
                                         tags$li(tags$strong("density_cluster"), ": 'What state is this cell in?'"),
                                         tags$li(tags$strong("branch_pred"), ": 'Which lineage is this cell heading toward?'"),
                                         tags$li(tags$strong("branch_segment_clusters"), ": 'What stage within the lineage?'"),
                                         tags$li(tags$strong("pseudotime"), ": 'How far along is differentiation?'")
                                       ),
                                       tags$h5("Results:"),
            ...
            tabPanel("Nichenet",  fluidRow(
                 box(
                   ...
                   selectInput(inputId = "NichenetDEident", label = "Choose identity containing comparing conditions (e.g., WT and KO): ", c("Cluster" = "orig.ident")),
                    ...
                    checkboxInput("includeAcrossCell", label = "Include condition-specificity of the ligand/receptor across all cell-types?", value = TRUE),###need to add code
                 ),
                 box(
                   ...
                   div(
                     tabsetPanel(type = "tabs",
                              ...
                              tabPanel("Circos plot",
                                    ...
                                    tags$h4("The determination of general/specific in Circosplot is performed on all cell types. If you want to determine only for the cells specified on the ligand side, analyze using a dataset with those other cells excluded."),
      ...
      tabItem(tabName = "grn",
          ...
          #           tags$h3("Prepare files for pyscenic"),
                 ...
                 box(width = 9, status = "info", solidHeader = TRUE, title = "GRN output",
                     ...
                     tags$br(),
                     tags$h5(HTML('Basically proceed in order 1, 2, 3. Data can be saved anytime.<br>Except for 1a, always specify identity with clusters to compare.<br>If there are many cells and it takes time, create metacells with SEACells in Metis, load SEAcells.summarized.h5ad data, normalize and perform 1a, download reg.csv from Download SCENIC res. Load original metacell-free data, upload reg.csv in 1b and analyze.<br>For datasets with multiple conditions like WT and KO, results may differ if performed on whole or individual data. When comparing WT and KO, for example, perform 1a on whole dataset, then perform 1b→2 on whole dataset, WT only, KO only and compare results.')),
                     tags$h5(HTML('When merging multiple datasets or using Hashtags, you can create reg.csv for the whole and then perform step 2 onwards on individual subsets.
                      <br>You can also create regulon.csv for the whole WT+KO and analyze WT and KO individually.')),
                      tags$h5(HTML('About transcription factor activity<br>
                      	Z-score: Activity score of regulon in each cell type.<br>
RSS: Entropy-based evaluation of how concentrated regulon activity is in specific cell types. If regulon is active only in one cell type, RSS score is 1. Range: 0-1.<br>Use RSS to evaluate specificity, Z-score to see relative strength of activity. Generally look at RSS, but activity can be low even when RSS is high. https://doi.org/10.1016/j.celrep.2018.10.045')),
                     tags$hr(),
